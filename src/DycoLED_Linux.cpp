...
{
    ubled_strip.set_pattern(led,preset_pattern[patt].color,preset_pattern[patt].brightness,preset_pattern[patt].time,
...
{//Use it main
 //Function that writess pattern that user has selected, generates beat for refreshing.
    ...
    for (int i=0; i<number_led; i++){
	 ...
	 ubled_strip.set_pattern(i,preset_pattern[patt].color,preset_pattern[patt].brightness,preset_pattern[patt].time,
                             preset_pattern[patt].res,preset_pattern[patt].len);
    }
